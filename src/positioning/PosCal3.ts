...
export class PosCal3 {
  ...
  static getMessageWidthAndParticipantWidth(ctx: any, widthProvider: WidthFunc): ICoordinates2 {
    ...
    return participantModels.map((p: IParticipantModel) => {
      ...
      const messageWidth = PosCal3.getMessageWidth(widthProvider, myMessage?.ownableMessages || [], leftNeighbour);
  ...
  private static getMessageWidth(widthProvider: WidthFunc, ownableMessages: Array<OwnableMessage>, leftNeighbour: string | undefined): number {
    const contributingMessages = ownableMessages?.filter(o => o.from === leftNeighbour);
