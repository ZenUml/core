...
test('Keyword "return" - in method block', () => {
  ...
  let returnedValue = rootContext.block().stat()[0].message().braceBlock().block().stat()[0].ret().value();
...
test('Keyword "return" - in alt block', () => {
  ...
  expect(seqDsl.RootContext).not.toBeNull()
  let returnedValue = rootContext.block().stat()[0].alt().ifBlock().braceBlock().block().stat()[0].ret().value();
...
test('Keyword "return" - in loop block', () => {
  ...
  expect(seqDsl.RootContext).not.toBeNull()
  let returnedValue = rootContext.block().stat()[0].loop().braceBlock().block().stat()[0].ret().value();
